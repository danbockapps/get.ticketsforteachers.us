...
export async function POST(request: NextRequest) {
  ...
  if (STOP_KEYWORDS.has(keyword)) {
    await db.update(users).set({smsConsentAt: null}).where(eq(users.phone, from))
    await logAction(`sms opt-out from ${from}`)
  } else if (START_KEYWORDS.has(keyword)) {
    // Affirmative re-subscribe. Only re-grant consent for a known, phone-verified user.
    const rows = await db
      .select({id: users.id, phoneVerified: users.phoneVerified})
      .from(users)
      .where(eq(users.phone, from))
    if (rows[0]?.phoneVerified) {
      await db
        .update(users)
        .set({smsConsentAt: new Date().toISOString()})
        .where(eq(users.phone, from))
    }
